...
# FlowDraft Animated Architecture Diagram

**FlowDraft 动态架构图: premium hand-drawn animated architecture diagrams for articles, systems, and workflows.**
...
[![Animated GIF](https://img.shields.io/badge/Animated-GIF-FFB000?style=for-the-badge)](./scripts/render_flowdraft_diagram.py)
...
`flowdraft-animated-architecture-diagram` is a Codex skill and local renderer for creating premium black-canvas technical diagrams with hand-drawn typography, editable Excalidraw output, static PNG previews, and genuinely animated GIFs.
...
```bash
git clone https://github.com/cclank/flowdraft-animated-architecture-diagram.git
cd flowdraft-animated-architecture-diagram
python3 -m pip install -r requirements.txt
python3 scripts/render_flowdraft_diagram.py \
...
```bash
~/.codex/skills/flowdraft-animated-architecture-diagram
```

Typical local install path:

```bash
${CODEX_HOME:-$HOME/.codex}/skills/flowdraft-animated-architecture-diagram
```
...
```text
Use $flowdraft-animated-architecture-diagram to turn this article into a premium hand-drawn animated architecture GIF.
```

Chinese prompt example:

```text
用 $flowdraft-animated-architecture-diagram 把这篇文章整理成 FlowDraft 动态架构图，输出 GIF、PNG 和 Excalidraw。
```
