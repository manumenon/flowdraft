...
def convert_legacy_spec_to_dynamic(spec: dict) -> dict:
    ...
    # Panels
    nodes.append({"id": "input_panel", "x": input_panel_x, "y": input_panel_y, "width": input_panel_w, "height": input_panel_h, "type": "panel", "title": spec.get("input_title", "Source / Input")})
    nodes.append({"id": "core_panel", "x": core_panel_x, "y": core_panel_y, "width": core_panel_w, "height": core_panel_h, "type": "panel", "title": core.get("title", "Archive Core")})
    nodes.append({"id": "left_panel", "x": left_panel_x, "y": left_panel_y, "width": left_panel_w, "height": left_panel_h, "type": "panel", "title": left_spec.get("title", "Memory Sources")})
    nodes.append({"id": "center_panel", "x": center_panel_x, "y": center_panel_y, "width": center_panel_w, "height": center_panel_h, "type": "panel", "title": center_spec.get("title", "Archive Layers")})
    nodes.append({"id": "right_panel", "x": right_panel_x, "y": right_panel_y, "width": right_panel_w, "height": right_panel_h, "type": "panel", "title": right.get("title", "Memory Pack")})

    # Input chips
    for i, inp in enumerate(inputs_data):
        nodes.append({"id": f"input_{i}", "x": resolved_input_centers[i] - inp["w"] / 2, "y": input_panel_y + 42, "width": inp["w"], "height": inp["h"], "type": "input", "title": inputs[i].get("label", ""), "icon": inp["icon"], "color": inp["color"]})

    # Core cards
    for i in range(n_cards):
        nodes.append({"id": f"core_card_{i}", "x": resolved_card_x[i], "y": card_y, "width": core_cards_data[i]["w"], "height": core_cards_data[i]["h"], "type": "card", "title": cards[i].get("title", ""), "body": cards[i].get("body", ""), "icon": core_cards_data[i]["icon"], "color": core_cards_data[i]["color"]})

    # Decision diamond
    nodes.append({"id": "decision", "x": decision_x, "y": decision_y, "width": dec_w, "height": dec_h, "type": "diamond", "title": dec_title, "body": dec_body})

    # Output card
    nodes.append({"id": "output", "x": output_x, "y": output_y, "width": out_w_final, "height": out_h_final, "type": "card", "title": out_label, "body": "", "icon": out_spec.get("icon", "file"), "color": THEME["cyan"]})

    # Left cards
    for i in range(len(left_cards_raw)):
        nodes.append({"id": f"left_card_{i}", "x": left_card_x, "y": left_card_ys[i], "width": left_cards_data[i]["w"], "height": left_cards_data[i]["h"], "type": "card", "title": left_cards_raw[i].get("title", ""), "body": left_cards_raw[i].get("body", ""), "icon": left_cards_data[i]["icon"], "color": left_cards_data[i]["color"]})

    # Center cards
    for i in range(len(layer_cards_raw)):
        nodes.append({"id": f"center_card_{i}", "x": layer_card_xs[i], "y": layer_card_y, "width": layer_cards_data[i]["w"], "height": layer_cards_data[i]["h"], "type": "card", "title": layer_cards_raw[i].get("title", ""), "body": layer_cards_raw[i].get("body", ""), "icon": layer_cards_data[i]["icon"], "color": layer_cards_data[i]["color"]})

    # Center footer card
    nodes.append({"id": "center_footer", "x": footer_x, "y": footer_y, "width": footer_w, "height": footer_h, "type": "card", "title": footer_text, "body": "", "icon": "file", "color": THEME["purple"]})

    # Right cards
    for i in range(len(right_cards_raw)):
        nodes.append({"id": f"right_card_{i}", "x": right_card_x, "y": right_card_ys[i], "width": right_cards_data[i]["w"], "height": right_cards_data[i]["h"], "type": "card", "title": right_cards_raw[i].get("title", ""), "body": right_cards_raw[i].get("body", ""), "icon": right_cards_data[i]["icon"], "color": right_cards_data[i]["color"]})

    connections = []
    # 1. Middle input chip -> core_card_0
    mid_input_idx = n_inputs // 2
    connections.append([f"input_{mid_input_idx}", "core_card_0"])
    # 2. core card connectors
    for i in range(n_cards - 1):
        connections.append([f"core_card_{i}", f"core_card_{i+1}"])
    # 3. core_card_last -> decision
    connections.append([f"core_card_{n_cards-1}", "decision"])
    # 4. decision -> output
    connections.append(["decision", "output"])
    # 5. output -> right_card_0
    connections.append(["output", "right_card_0"])
    # 6. decision -> core_card_0
    connections.append(["decision", "core_card_0"])
    # 7. left_card_0 -> core_card_0
    connections.append(["left_card_0", "core_card_0"])
    # 8. core_card_2 -> center_card_2 (embeddings to db)
    if n_cards > 2:
        connections.append(["core_card_2", "center_card_2"])
    # 9. core_card_3 -> center_card_3 (graph to db)
    if n_cards > 3:
        connections.append(["core_card_3", "center_card_3"])
    # 10. center_card_3 -> right_card_0
    connections.append(["center_card_3", "right_card_0"])
    # 11. right_card_0 -> decision
    connections.append(["right_card_0", "decision"])
    
    # 12. left_cards internal flow
    for i in range(len(left_cards_raw) - 1):
        connections.append([f"left_card_{i}", f"left_card_{i+1}"])
    # 13. right_cards internal flow
    for i in range(len(right_cards_raw) - 1):
        connections.append([f"right_card_{i}", f"right_card_{i+1}"])
    # 14. center_card_1 -> center_footer
    connections.append(["center_card_1", "center_footer"])
...
def route_connection_path(
    ...
    # --- 4. Orthogonal Routing with Obstacle Crossing Avoidance ---
    is_start_horiz = dir_start in ("left", "right")
    
    preferred_shape = "A" if is_start_horiz else "B"
    shapes_to_try = ["A", "B"] if preferred_shape == "A" else ["B", "A"]
    ...
    def path_intersects_obstacles(path):
        for node in nodes:
            if node["id"] in (id_a, id_b) or node.get("type") == "panel":
                continue
            nx1 = node["x"] - margin
            ny1 = node["y"] - margin
            nx2 = node["x"] + node["width"] + margin
            ny2 = node["y"] + node["height"] + margin
            
            for p1, p2 in zip(path, path[1:]):
                if abs(p1[0] - p2[0]) < 1e-2:
                    x = p1[0]
                    y1_seg = min(p1[1], p2[1])
                    y2_seg = max(p1[1], p2[1])
                    if nx1 <= x <= nx2 and not (y2_seg < ny1 or y1_seg > ny2):
                        return True
                elif abs(p1[1] - p2[1]) < 1e-2:
                    y = p1[1]
                    x1_seg = min(p1[0], p2[0])
                    x2_seg = max(p1[0], p2[0])
                    if ny1 <= y <= ny2 and not (x2_seg < nx1 or x1_seg > nx2):
                        return True
                else:
                    min_x = min(p1[0], p2[0])
                    max_x = max(p1[0], p2[0])
                    min_y = min(p1[1], p2[1])
                    max_y = max(p1[1], p2[1])
                    if not (max_x < nx1 or min_x > nx2 or max_y < ny1 or min_y > ny2):
                        return True
        return False

    for shape in shapes_to_try:
        if shape == "A":
            X_default = (P_start[0] + P_end[0]) / 2
            candidates = [X_default]
            for node in nodes:
                if node["id"] not in (id_a, id_b) and node.get("type") != "panel":
                    candidates.append(node["x"] - margin - 2)
                    candidates.append(node["x"] + node["width"] + margin + 2)
            candidates = sorted(list(set(candidates)), key=lambda x: abs(x - X_default))
            
            for X in candidates:
                path = [P_start, (X, P_start[1]), (X, P_end[1]), P_end]
                if not path_intersects_obstacles(path):
                    routed_path = path
                    break
        else:
            Y_default = (P_start[1] + P_end[1]) / 2
            candidates = [Y_default]
            for node in nodes:
                if node["id"] not in (id_a, id_b) and node.get("type") != "panel":
                    candidates.append(node["y"] - margin - 2)
                    candidates.append(node["y"] + node["height"] + margin + 2)
            candidates = sorted(list(set(candidates)), key=lambda y: abs(y - Y_default))
            
            for Y in candidates:
                path = [P_start, (P_start[0], Y), (P_end[0], Y), P_end]
                if not path_intersects_obstacles(path):
                    routed_path = path
                    break
        if routed_path:
    ...
    if not routed_path:
        if preferred_shape == "A":
            X_mid = (P_start[0] + P_end[0]) / 2
            routed_path = [P_start, (X_mid, P_start[1]), (X_mid, P_end[1]), P_end]
        else:
            Y_mid = (P_start[1] + P_end[1]) / 2
            routed_path = [P_start, (P_start[0], Y_mid), (P_end[0], Y_mid), P_end]

    # --- 5. Apply Parallel Overlapping Path Offsets ---
    key = tuple(sorted([id_a, id_b]))
    group = segment_groups.get(key, [])
    if len(group) > 1:
        idx_in_group = 0
        ...
        P0, P1, P2, P3 = routed_path
        if abs(P0[1] - P1[1]) < 1e-2:
            routed_path = [
                (P0[0], P0[1] + L),
                (P1[0] + L, P1[1] + L),
                (P2[0] + L, P2[1] + L),
...
def main():
    ...
    if "title" in spec:
        ...
        if prefix_text:
            draw_text(ex_doc, draw, prefix_text, 45, 14, 535, 66, 47, THEME["white"], "left", hand=True, bold=True, scaled=False)
        if highlight_text:
            ...
            draw_text(ex_doc, draw, highlight_text, hl_rect_x + _hl_pad_x, 19, hl_rect_w - _hl_pad_x * 2, 76, 44, THEME["green"], "center", hand=True, bold=True, scaled=False)
        if subtitle_text:
    ...
    # 3. Draw Signature/Brand watermark
    ...
    for node in nodes:
        ...
        if ntype == "panel":
            ...
            if node["id"] == "left_panel":
                draw_text(ex_doc, draw, ntitle, nx + 19, ny + 17, nw - 38, 30, 22, THEME["white"], "left", hand=hand_font, bold=bold_font, fit=True, min_size=12, scaled=False)
            else:
            ...
            if not spec_had_nodes:
                if node["id"] == "left_panel" and "left_panel" in spec:
                    ...
                    if badge:
                        draw_text(ex_doc, draw, badge, nx + nw - 80, ny + 44, 62, 18, 11, THEME["green"], "center", scaled=False)
                elif node["id"] == "center_panel" and "center_panel" in spec:
                    sub = spec["center_panel"].get("subtitle")
                    if sub:
                        draw_text(ex_doc, draw, sub, nx + (nw - 300) / 2, ny + 56, 300, 24, 14, THEME["white"], "center", scaled=False)
                elif node["id"] == "core_panel" and "core" in spec:
                    sub = spec["core"].get("subtitle")
                    if sub:
                        draw_text(ex_doc, draw, sub, nx + 390, ny + 38, 720, 20, 11, THEME["muted"], "left", scaled=False)
            ...
            hand_font = True if "core_card" in node["id"] else style["hand"]
            bold_font = style.get("bold", True)
            ...
            title_h = nh * 0.3
            body_h = nh * 0.4
            draw_text(ex_doc, draw, ntitle, nx + nw * 0.15, ny + nh * 0.25, nw * 0.7, title_h, 18, THEME["white"], "center", hand=style["hand"], bold=True, fit=True, min_size=10, scaled=False)
            if nbody:
                draw_text(ex_doc, draw, nbody, nx + nw * 0.15, ny + nh * 0.25 + title_h + 3, nw * 0.7, body_h, 13, THEME["white"], "center", hand=style["hand"], bold=style["bold"], fit=True, min_size=10, scaled=False)
        elif ntype == "text":
    ...
    if not spec_had_nodes:
        ...
        reusable_x = p_loop_start[0] + (right_panel_node["x"] - p_loop_start[0] - 75) / 2
        ...
        _lbl_gap = right_panel_node["x"] - (center_panel_node["x"] + center_panel_node["width"])
        _lbl_w   = max(40, _lbl_gap - 8)
        _lbl_x   = center_panel_node["x"] + center_panel_node["width"] + (_lbl_gap - _lbl_w) / 2
        draw_text(ex_doc, draw, spec.get("right_panel", {}).get("incoming_label", "Compile"), _lbl_x, center_panel_node["y"] + 116, _lbl_w, 44, 12, THEME["white"], "center", fit=True, min_size=9, scaled=False)
    ...
    for p, conn_dict in enumerate(normalized_connections):
        ...
        for seg_idx in range(len(conn) - 1):
            ...
            if conn_label and seg_idx == 0:
                ...
                draw_text(ex_doc, draw, conn_label, lbl_x - 50, lbl_y - 10, 100, 20, 12, stroke, "center", hand=fc.HAND, scaled=False)
